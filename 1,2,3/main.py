...
class ImageToAI():
    ...
    def create_bin_matrix(self, delta: float, flg: bool) -> tuple[int]:
        ...
        self.EV = (np.mean(self.bin_matrix, axis=0) >= 0.5).astype(int)
        if flg:
...
class Ex(tk.Tk):
    ...
    def create_binary_matrix(self):
        ...
        def save_params():
            with open("bin_matrix.txt", "w") as fd:
                fd.write("Binary matrix:\n")
                for line in self.input_image.bin_matrix:
                    res = ", ".join(str(line).split())
                    fd.write(f"{res}\n")
                fd.write(f"\nEV:\n{self.input_image.EV}")
                tk.Label(self.second_task, text="✅").grid(row=10,
                                                          column=1)

        """Вывод файла с значениями"""
        tk.Button(self.second_task, text="Вывести полные данные", 
                  command=save_params).grid(row=10, column=0)
